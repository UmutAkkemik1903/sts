...
const Edit = (props) => {
    ...
    const handleSubmit = (values,{ resetForm,setSubmitting }) => {
        ...
        newImages.forEach((image_file)=>{
            if (typeof image_file.id != 'number'){
        ...
        axios.post(`/api/product/${product.id}`,data,config).then((res)=>{
            if(res.data.success){
                toastr.success('Kayıt Güncellendi','Ürün')
                setSubmitting(false)
            }
            else
            {
                toastr.warning('Kayıt Güncellenemedi','Ürün')
                swal(res.data.message);
            }
        }).catch(e => console.log(e));
    };
